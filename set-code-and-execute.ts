...
const main = async () => {
  ...
  const CONTRACT_IMPL = process.env.CONTRACT_IMPL;
  const ContractImpl = await ethers.getContractAt("ContractImpl", CONTRACT_IMPL);

  console.log("Chain ID: ", chainId);
  console.log("EOA address: ", wallet.address);
  console.log("Setting code for EIP7702 account at: ", CONTRACT_IMPL);

  // Encode the execute function call with ContractImpl.functionToCall()
  // const calldata = ContractImpl.interface.encodeFunctionData("functionToCall");
